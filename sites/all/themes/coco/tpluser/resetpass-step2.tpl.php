...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
      ...
      <tbody>
        <tr>
          <td class="steps" colspan="2">
            ...
            <b>&gt;</b>
            <span class="step current"><i>2</i> 验证+重置密码</span>
          </td>
        </tr>
        <tr>
          ...
          <td class="item">
            ...
            <button class="sendmail" type="button" onclick="resetpass_apply(<?php echo $account->uid;?>)">免费获取验证码</button>
          </td>
        </tr>
        <tr>
          <td class="label">输入验证码：</td>
          <td class="item">
            <input type="text" name="resetpasscode" value=""/>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td class="label"></td>
          <td class="action">
            <input type="submit" onclick="resetpass_validate(<?php echo $account->uid;?>)" value="确 定"/>
    ...
    <script type="text/javascript">
      function resetpass_apply(uid){
        $('#formtable .status').text('');
        var url = Drupal.settings.basePath + 'cmsuser/user/0/resetpass';
      ...
      function resetpass_validate(uid){
        $('#formtable .status').text('');
        var resetpasscode = $('input[name=resetpasscode]').val();
        var pass = $('input[name=pass]').val();
        var url = Drupal.settings.basePath + 'cmsuser/user/0/resetpass';
        $.post(url, {uid:uid, action:'validate', resetpasscode:resetpasscode, pass:pass}, function(ret){
          if(ret.status === 'success'){
            $('#formtable .status').css('color', 'green');
            $('#formtable .status').html(ret.text + ' <a style="vertical-align: bottom;" href="' + Drupal.settings.basePath + 'ulogin">【点击登录】</a>');
            $('input[name=resetpasscode]').val('');
            $('input[name=pass]').val('');
          }else{
            $('#formtable .status').css('color', 'red');
            $('#formtable .status').text(ret.text);
