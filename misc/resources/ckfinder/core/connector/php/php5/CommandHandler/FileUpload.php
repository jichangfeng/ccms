...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        /* rename file name start */
        $sExtension = CKFinder_Connector_Utils_FileSystem::getExtension($sFileName);
        $sFileName = date("YmdHis") . rand(10000, 99999) . "." . $sExtension;
        /* rename file name end */
        ...
        /* create timestamp folder start */
        $datenow = date('Ymd', time());
        $sServerDir_arr = explode('ckfinder', $sServerDir); 
        $sServerDir_pop = array_pop($sServerDir_arr);
        $timeDir = $sServerDir . '/' . $datenow . '/';
        if (!file_exists($timeDir) && !is_dir($timeDir)){
          mkdir($timeDir, 0777);	
        }
        $sServerDir = $timeDir;
        $oRegistry->set("FileUpload_url", $this->_currentFolder->getUrl() . $datenow . '/');
        /* create timestamp folder end */
        $iCounter = 0;
